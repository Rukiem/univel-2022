...
# EXAMPLE:
## Scores = [12,12,23,42,14,13]
## upgrade = [num + 2 for num in Scores]
## print(upgrade)

## EXAMPLE 2
## List comprehension can contain only the if part of the conditionals
# Scores = [12,12,23,42,14,13]
# upgrade = [num + 2 for num in Scores if num % 2 != 0]
# print(upgrade)
# a list comprehension always returns a list as long as it does not contain print in it's expression

# doe = "3"
# date = dt.strptime(doe, "%m")
# gas = date.strftime("%B")
# print(gas)

# dare = list(map(lambda a: str(a), list_of_date))
# dae = list(map(lambda s : s.split("/")[0], dare))
# print(dae)
# print(len(dae))
# gas = list(map(lambda a :dt.strptime(a, "%m"), dae))
# date = list(map(lambda s:s.strftime("%B"), gas))
# print(date)

# next = {}
# for month, amount in zip(list_of_date , list_sales):
#     next[month] = next.get(month, 0) + amount
